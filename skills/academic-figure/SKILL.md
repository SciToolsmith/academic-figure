...
5. 目标读者、用途（投稿、汇报、教学或探索）、最终尺寸和输出格式。
6. 成图可见文字的语言；它与 Python/R 绘图后端分开判断。
...
- 用户明确指定时服从其选择；修改已有图或服务于既有稿件时，默认保持原图或稿件的主语言。
- 否则根据用户提供的标题、轴标签、数据术语和当前请求语言判断。只有参考图同时承担内容参考时才继承其主语言；仅作风格参考的图和英文案例模板不能决定输出语言。
- 不机械翻译基因名、物种名、模型名、变量 ID、单位和标准缩写。中英混排或语言选择会实质影响交付时，才提出一个简短问题。

有真实数据时必须使用真实数据，不得用模拟数据替换。没有数据且用户希望看可运行演示时，可使用固定随机种子的中性模拟数据，并在回答、代码或交付说明中明确其模拟状态。投稿成图默认不显示随机种子、演示声明或运行说明；只有用户明确要求独立传播的教学/演示图时，才可在图内放简短的“模拟数据”标识。不得将模拟数据解读为真实结论，也不得在真实数据读取失败后静默切换。
...
需要提供多个方案时，在回答或单独的对比预览中标注“方案一/二/三”及理由；每个单独导出的投稿候选图保持清洁，不把方案名称、设计解释或复现说明写进画布。
